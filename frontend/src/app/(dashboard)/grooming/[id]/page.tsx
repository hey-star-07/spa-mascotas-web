...
import { Dog, Scissors, Camera, CheckCircle, XCircle, Lock, AlertTriangle, Save, ClipboardList, MessageSquare, User, Clock, Calendar, Package } from "lucide-react";
...
export default function FichaTecnicaPage() {
  ...
  const cerrarFicha = async () => {
    ...
    if (completados < total) {
      const pendientes = ficha?.checklist.filter(c => !c.completado).map(c => c.plantillaChecklist.item).join(', ');
      return toast.error(`Completa todos los items del checklist. Pendientes: ${pendientes}`);
    }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" onClick={() => router.push("/groomer-dashboard")}>← Volver</Button>
          <div>
            <h1 className="text-3xl font-extrabold flex items-center gap-2">
              <Scissors className="h-8 w-8" strokeWidth={3} />Ficha Técnica
            </h1>
        ...
        {!ficha.fechaCierre ? (
          completados < total ? (
            <div className="text-right">
              <Button disabled variant="secondary" size="lg" className="opacity-50 cursor-not-allowed">
                <Lock className="mr-2 h-5 w-5" /> Completar Checklist ({completados}/{total})
              </Button>
              <p className="text-xs text-rose font-semibold mt-1">Completa los {total - completados} ítems restantes para cerrar</p>
            </div>
          ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card><CardContent className="pt-4 text-center"><Badge variant={completados === total ? "default" : "secondary"}>{completados}/{total}</Badge><p className="text-xs mt-1">Checklist</p></CardContent></Card>
      </div>
        ...
        <CardContent>
          ...
          {ficha.cita.mascota.alergiasConocidas && (
            <div className="bg-rose/10 border-2 border-rose rounded-xl p-3 flex items-start gap-2">
              ...
              <div><p className="text-sm font-extrabold text-rose">Alergias</p><p className="text-xs">{ficha.cita.mascota.alergiasConocidas}</p></div>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge variant={completados === total ? "default" : "secondary"}>{completados}/{total}</Badge>
          </div>
          <div className="mt-3">
            <div className="flex gap-1">
              {Array.from({ length: total }).map((_, i) => (
                <div key={i} className={`h-2.5 flex-1 rounded-full border-2 border-foreground transition-all ${i < completados ? (completados === total ? "bg-primary" : "bg-accent") : "bg-gray-200"}`} />
              ))}
            </div>
            <div className="flex justify-between mt-1 text-[10px] text-foreground/50">
              <span>{completados === total ? "✅ Listo" : `${total - completados} pendiente(s)`}</span>
        ...
        <CardContent>
          <div className="space-y-2">
            {ficha.checklist.map((item) => (
              <div key={item.id} className={`flex items-center justify-between p-3 border-2 border-foreground rounded-xl ${item.completado ? "bg-primary/20" : "bg-white"}`}>
                <div className="flex items-center gap-3 flex-1">
                  <button onClick={() => !ficha.fechaCierre && toggleChecklist(item.id, item.completado)} disabled={!!ficha.fechaCierre}>
                    {item.completado ? <CheckCircle className="h-6 w-6 text-primary" /> : <div className="h-6 w-6 rounded-full border-3 border-foreground/30" />}
                  </button>
                  <span className={`font-semibold text-sm ${item.completado ? "line-through text-foreground/50" : ""}`}>{item.plantillaChecklist.item}</span>
                </div>
              </div>
            ))}
          </div>
          {!ficha.fechaCierre && completados < total && (
            <div className="mt-4 bg-rose/10 border-2 border-rose rounded-xl p-3">
              <p className="text-sm font-extrabold text-rose">Faltan {total - completados} ítem(es)</p>
            </div>
          )}
          {!ficha.fechaCierre && completados === total && total > 0 && (
            <div className="mt-4 bg-primary/20 border-2 border-primary rounded-xl p-3 text-center">
              ...
              <p className="text-sm font-extrabold">¡Checklist completo!</p>
            </div>
          )}
        </CardContent>
      </Card>

      {/* Fotos, Insumos, Recomendaciones (versión resumida) */}
      <Card>
        <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Camera className="h-5 w-5" />Fotos</CardTitle></CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-4 mb-4">
            {ficha.fotos.map((foto) => (
              <div key={foto.id} className="relative"><Badge className="absolute top-2 left-2 z-10">{foto.tipo}</Badge><img src={getImageUrl(foto.urlFoto) || ''} className="w-32 h-32 object-cover rounded-xl border-3 border-foreground" /></div>
            ))}
          </div>
          {!ficha.fechaCierre && (
            <div className="flex gap-3 items-end">
              <select value={fotoTipo} onChange={e => setFotoTipo(e.target.value as any)} className="h-12 rounded-xl border-3 border-foreground bg-white px-4 font-bold"><option value="antes">Antes</option><option value="despues">Después</option></select>
              <Input value={fotoUrl} onChange={e => setFotoUrl(e.target.value)} placeholder="URL foto..." className="flex-1" />
      ...
      {/* Insumos */}
      <Card>
        <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Package className="h-5 w-5" />Insumos</CardTitle></CardHeader>
        <CardContent>
            ...
            <div className="space-y-2 mb-4">
              {ficha.consumoInsumos.map((insumo) => (
                <div key={insumo.id} className="flex items-center justify-between p-2 border-2 border-foreground rounded-xl text-sm">
                  ...
                  <span>{Number(insumo.cantidad)} uds</span>
                  <div className="flex gap-1">
                    {!ficha.fechaCierre && (
                      <>
